...
# Description of the sort parameters for wget command.
# -s: Silent mode.
...
if [ -n "$GITHUB_API_TOKEN" ] && [ "${#GITHUB_API_TOKEN}" -gt 36 ]; then
    CURL_OUTPUT="$(wget -q --timeout=$TIMEOUT --header="Authorization: Bearer $GITHUB_API_TOKEN" -O - https://api.github.com/repos/${GH_REPO}/releases/latest)"
    EXIT_CODE=$?
else
    CURL_OUTPUT="$(wget -q --timeout=$TIMEOUT -O - https://api.github.com/repos/${GH_REPO}/releases/latest)"
    EXIT_CODE=$?
fi
